...
{
    /// <summary>
    /// 아쿠아리움 시스템을 프리팹 없이 조작·확인하는 IMGUI 테스트 패널. GameObject에 컴포넌트만 붙이면 동작한다.
    /// 표시·조작 항목:
    ///  - 인벤토리 물고기를 개체 단위로 나열하고 [배치] 버튼과 불가 사유를 표시한다.
    ///  - 배치된 물고기와 다음 생산까지 남은 시간, [회수] 버튼과 선검사 사유를 표시한다.
    ///  - 생산 중인 재료의 누적/요구 포인트, 시간당 개수, 보류 수를 표시한다.
    ///  - 수용량(사용/최대)·등급과 강화 버튼, 골드·재료 비용과 부족 사유를 표시한다.
    /// IMGUI라 에디터 Play에서만 조작할 수 있다.
    /// </summary>
