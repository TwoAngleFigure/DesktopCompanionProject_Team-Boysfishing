...
## 4. 작업 권한 및 폴더 접근 규칙
- **1_KSS 폴더 전용**: 코드 수정 및 생성은 반드시 1_KSS 폴더 내에서만 이루어져야 합니다.
- **타 폴더 읽기 전용**: 1_KSS 이외의 다른 폴더에 있는 파일들은 코드 흐름 파악을 위해 읽기만 가능하며, 절대 수정하거나 새로운 파일을 생성해서는 안 됩니다. (다른 팀원들의 코드를 보호하기 위함입니다.)
